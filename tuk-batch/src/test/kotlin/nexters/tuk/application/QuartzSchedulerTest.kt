...
class QuartzSchedulerTest @Autowired constructor(
    ...
    fun `새로운 meetingId로 알림을 등록하면 Job과 Trigger가 생성된다`() {
        ...
        val command = NotificationCommand.Reservation(meetingId, sendAt)
        ...
        meetingScheduler.scheduleNotification(command)
    ...
    fun `동일 meetingId로 재등록 시 기존 Trigger가 새 정보로 교체된다`() {
        ...
        meetingScheduler.scheduleNotification(NotificationCommand.Reservation(meetingId, initialTime))
        meetingScheduler.scheduleNotification(NotificationCommand.Reservation(meetingId, updatedTime))
    ...
    fun `여러 meetingId로 등록 시 각 스케줄이 독립적으로 존재한다`() {
        val command1 = NotificationCommand.Reservation(10L, LocalDateTime.now().plusMinutes(10))
        val command2 = NotificationCommand.Reservation(20L, LocalDateTime.now().plusMinutes(20))

        meetingScheduler.scheduleNotification(command1)
        meetingScheduler.scheduleNotification(command2)
