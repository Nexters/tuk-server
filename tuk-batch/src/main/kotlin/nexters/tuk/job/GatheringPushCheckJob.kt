...
class GatheringPushCheckJob(
    ...
    private val logger = LoggerFactory.getLogger(GatheringPushCheckJob::class.java)

    override fun execute(context: JobExecutionContext) {
        logger.info("[GatheringPushCheckJob] 모임 푸시 알림 체크 시작")
        val gatherings = gatheringRepository.getAllGatherings()
                ...
                pushApiClient.sendPushNotification(
                    PushDto.Push(
                        recipients = gathering.memberIds.map { memberId ->
                            PushDto.PushRecipient(
                                memberId = memberId,
                                deviceToken = "device_token_$memberId" // 실제 디바이스 토큰을 조회하여 사용해야 힘
                            )
                        },
