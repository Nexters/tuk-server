...
@SpringBootTest
class GatheringFacadeIntegrationTest @Autowired constructor(
    ...
    fun `모임 생성시 모임과 관련된 정보들이 정상적으로 저장된다`() {
        ...
        // when
        val result = gatheringFacade.generateGathering(command)
    ...
    fun `유저의 모임 목록을 정상적으로 조회한다`() {
        ...
        // when
        val result = gatheringFacade.getMemberGatherings(
    ...
    fun `다른 유저의 모임은 조회되지 않는다`() {
        ...
        gatheringFacade.generateGathering(GatheringFixture.gatheringGenerateCommand(member1.id, "gathering1"))
        gatheringFacade.generateGathering(GatheringFixture.gatheringGenerateCommand(member2.id, "gathering2"))
    ...
    fun `모임 이름 오름차순으로 정렬되어 조회된다`() {
        ...
        listOf("A_gathering", "B_gathering", "C_gathering").forEach { name ->
            gatheringFacade.generateGathering(GatheringFixture.gatheringGenerateCommand(member.id, name))
        }
    ...
    fun `삭제된 모임은 조회되지 않는다`() {
        ...
        gatheringFacade.generateGathering(GatheringFixture.gatheringGenerateCommand(member.id, "gathering1"))
        val result2 =
            gatheringFacade.generateGathering(GatheringFixture.gatheringGenerateCommand(member.id, "gathering2"))
    ...
    fun `모임 상세 정보를 정상적으로 조회한다`() {
        ...
        val result = gatheringFacade.generateGathering(
            GatheringFixture.gatheringGenerateCommand(member.id, "test gathering", daysSinceLastGathering = 10)
        )

        // when
        val detailResult = gatheringFacade.getGatheringDetail(
    ...
    fun `모임에 속한 멤버들의 정보를 정상적으로 조회한다`() {
        ...
        val gatheringResult = gatheringFacade.generateGathering(
        ...
        // when
        val result = gatheringFacade.getGatheringDetail(
    ...
    fun `존재하지 않는 모임 ID로 조회 시 예외가 발생한다`() {
        ...
        assertThrows<RuntimeException> {
            gatheringFacade.getGatheringDetail(
    ...
    fun `다른 멤버들의 모임이 섞여있어도 본인의 모임만 조회된다`() {
        ...
        // generateGathering을 통해 모임을 생성하면 자동으로 호스트가 등록됨
        gatheringFacade.generateGathering(GatheringFixture.gatheringGenerateCommand(member1.id, "member1_gathering"))
        gatheringFacade.generateGathering(GatheringFixture.gatheringGenerateCommand(member2.id, "member2_gathering"))
        gatheringFacade.generateGathering(GatheringFixture.gatheringGenerateCommand(member3.id, "member3_gathering"))
        gatheringFacade.generateGathering(
    ...
    fun `서로 다른 간격의 모임들이 올바른 날짜 계산을 하는지 확인한다`() {
        ...
        val dailyResult = gatheringFacade.generateGathering(
            GatheringFixture.gatheringGenerateCommand(
                member.id, "daily", daysSinceLastGathering = 30, gatheringIntervalDays = 1
            )
        )
        val weeklyResult = gatheringFacade.generateGathering(
            GatheringFixture.gatheringGenerateCommand(
                member.id, "weekly", daysSinceLastGathering = 14, gatheringIntervalDays = 7
            )
        )
        val monthlyResult = gatheringFacade.generateGathering(
            GatheringFixture.gatheringGenerateCommand(
    ...
    fun `동시에 여러 모임을 생성해도 데이터 일관성이 유지된다`() {
        ...
        val results = commands.map { command: GatheringCommand.Generate ->
            gatheringFacade.generateGathering(command)
        }
        ...
        val memberGatherings = gatheringFacade.getMemberGatherings(
            GatheringQuery.MemberGathering(memberId = member.id)
        )
    ...
    fun `초대 수락시 멤버가 모임에 정상적으로 가입된다`() {
        ...
        // when
        val result = gatheringFacade.joinGathering(command)
    ...
    fun `존재하지 않는 모임 ID로 초대 수락시 NOT_FOUND 예외가 발생한다`() {
        ...
        val exception = assertThrows<BaseException> {
            gatheringFacade.joinGathering(command)
        }
